function write_demand_set_hot_xml(fid, sr_control, xlsx_file, range, or_id, ORS, dc, orgf2, orgf3, orgf4, warmup_time)
...
for i = 1:sz
    if or_id(i) ~= 0
        ...
        if isempty(ors)
            ...
            write_demand_profile_xml(fid, or_id(i), ORD(i, :), ORH(i, :), dc, warmup_time);
        else
...
function write_demand_profile_xml(fid, or_id, demand, hov_prct, dc, warmup_time)
% fid - file descriptor for the output xml
% or_id - on-ramp ID
...
fprintf(fid, '    <demand vehicle_type_id="2">0');
for i = 2:sz
    fprintf(fid, ',0');
end
fprintf(fid, '</demand>\n');
...
for j = 1:dc
  fprintf(fid, '    <demand vehicle_type_id="%d">0', j+2);
  for i = 2:sz
    fprintf(fid, ',0');
  end
  fprintf(fid, '</demand>\n');
